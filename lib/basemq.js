...
BaseMQ.prototype.use = function(name, opts) {
  switch (name) {
    case Module.CONNECTION:
      this._connection = _.defaults({}, this._connection, opts);
      break;
  }
...
BaseMQ.prototype.initialize = function() {
  ...
  _this._socket.on('message', function() {
    ...
    switch (_this._connection.approach) {
      case Connection.REQ:
      case Connection.REP:
        args.shift();
        break;
  ...
  if (this._connection.socket === Connection.CONNECT && this._connection.approach === Connection.REQ) {
...
BaseMQ.prototype.destroy = function() {
  ...
  switch (this._connection.socket) {
    case Connection.CONNECT:
      this._socket.disconnect(this._connection.endpoint);
      break;
    case Connection.BIND:
      this._socket.unbindSync(this._connection.endpoint);
      break;
  }
